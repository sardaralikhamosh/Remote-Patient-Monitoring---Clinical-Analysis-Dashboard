...
<head>
    ...
    <style>
        * {
        ...
        .upload-text { color:var(--text-secondary); font-size:11px; font-weight:500; text-align:center; line-height:1.5; }
        ...
        @media print {
            ...
            /* Hide everything that is interactive / control */
            ...
            .right-column { padding: 20px 24px; }
            .metric-row {
                display: grid;
                grid-template-columns: 75% 25%;
                border-bottom: 2px solid var(--border-color);
            ...
            /* Keep gauge canvases visible */
            canvas { display: block !important; }

            /* Traffic lights keep their colors */
            .light { box-shadow: inset 0 2px 8px rgba(0,0,0,0.3) !important; }
            .light.color-red    { box-shadow: 0 0 24px rgba(239,68,68,0.6), inset 0 2px 8px rgba(0,0,0,0.3) !important; }
            .light.color-yellow { box-shadow: 0 0 24px rgba(245,158,11,0.6), inset 0 2px 8px rgba(0,0,0,0.3) !important; }
            .light.color-green  { box-shadow: 0 0 24px rgba(16,185,129,0.6), inset 0 2px 8px rgba(0,0,0,0.3) !important; }

            /* Image shows at its current size, no controls */
            .graph-upload-section {
                border: 1px solid var(--border-color) !important;
                cursor: default !important;
            }
            .upload-placeholder { display: none !important; }
            .graph-preview { display: block !important; }
        }
...
<div class="dashboard-container">
    <div class="main-layout">
        ...
        <div class="left-column">
            ...
            <div class="graph-upload-section" id="graphUploadSection">
                <!-- placeholder shown when no image -->
                <div class="upload-placeholder" id="uploadPlaceholder" onclick="document.getElementById('graphUpload').click()">
                    <div class="upload-icon">📊</div>
                    <div class="upload-text">📋 Paste image (Ctrl+V) or click to upload<br><span style="opacity:0.7;">Snipping tool supported</span></div>
                </div>
                <input type="file" id="graphUpload" accept="image/*" onchange="handleGraphUpload(event)">

                <!-- shown when image exists -->
                <div class="img-wrapper" id="imgWrapper" style="display:none;">
                    <img id="graphPreview" class="graph-preview" src="" alt="Uploaded graph">
                    <div class="img-controls">
                        <label>Size</label>
                        <input type="range" id="sizeSlider" min="40" max="100" value="100" oninput="resizeImage(this.value)">
...
// ═══════════════════════════════════
//  IMAGE — paste (clipboard) + file upload + size slider
// ═══════════════════════════════════
function showImage(src) {
    const preview   = document.getElementById('graphPreview');
    const wrapper   = document.getElementById('imgWrapper');
    const placeholder = document.getElementById('uploadPlaceholder');
    const section   = document.getElementById('graphUploadSection');
...
function resizeImage(val) {
    document.getElementById('graphPreview').style.width = val + '%';
